...
namespace cutee {
  namespace common {
    ...
    struct interval {
      typedef T scalar_t;
      inline interval() 
        : lower(cutee::common::empty_box_lower<T>()),
          upper(cutee::common::empty_box_upper<T>())
      {}
      ...
      inline bool contains(const T &t) const { return t >= lower && t <= upper; }
      inline bool empty() const { return begin > end; }
      inline bool is_empty() const { return begin > end; }
      inline T center() const { return (begin+end)/2; }
      inline T span() const { return end - begin; }
      inline T diagonal() const { return end - begin; }
      inline interval<T> &extend(const T &t)
      { lower = min(lower,t); upper = max(upper,t); return *this; }
      inline interval<T> &extend(const interval<T> &t)
      { lower = min(lower,t.lower); upper = max(upper,t.upper); return *this; }
      inline interval<T> including(const T &t)
    ...
    template<typename T>
    inline interval<T> build_interval(const T &a, const T &b)
    ...
    struct box_t {
      ...
      inline box_t()
        : lower(cutee::common::empty_box_lower<typename T::scalar_t>()),
          upper(cutee::common::empty_box_upper<typename T::scalar_t>())
      {}

      // /*! construct a new, origin-oriented box of given size */
      // explicit inline box_t(const vec_t &box_size)
      //   : lower(vec_t(0)),
      //     upper(box_size)
      // {}
      /*! construct a new box around a single point */
      explicit inline box_t(const vec_t &v)
        : lower(v),
          upper(v)
      {}

      /*! construct a new, origin-oriented box of given size */
      inline box_t(const vec_t &lo, const vec_t &hi)
        : lower(lo),
          upper(hi)
      {}

      /*! returns new box including both ourselves _and_ the given point */
      inline box_t including(const vec_t &other) const
      { return box_t(min(lower,other),max(upper,other)); }
      /*! returns new box including both ourselves _and_ the given point */
      inline box_t including(const box_t &other) const
      { return box_t(min(lower,other.lower),max(upper,other.upper)); }

    
      /*! returns new box including both ourselves _and_ the given point */
      inline box_t &extend(const vec_t &other) 
      { lower = min(lower,other); upper = max(upper,other); return *this; }
      /*! returns new box including both ourselves _and_ the given point */
      inline box_t &extend(const box_t &other) 
      { lower = min(lower,other.lower); upper = max(upper,other.upper); return *this; }
    
    
      /*! get the d-th dimensional slab (lo[dim]..hi[dim] */
      inline interval<scalar_t> get_slab(const uint32_t dim)
      {
        return interval<scalar_t>(lower[dim],upper[dim]);
      }
    
      inline bool contains(const vec_t &point) const
      { return !(any_less_than(point,lower) || any_greater_than(point,upper)); }

      inline bool overlaps(const box_t &other) const
      { return !(any_less_than(other.upper,lower) || any_greater_than(other.lower,upper)); }

      inline vec_t center() const { return (lower+upper)/(typename vec_t::scalar_t)2; }
      inline vec_t span()   const { return upper-lower; }
      inline vec_t size()   const { return upper-lower; }

      inline typename long_type_of<typename T::scalar_t>::type volume() const
      { return cutee::common::volume(size()); }
    
      inline bool empty() const { return any_less_than(upper,lower); }
    ...
    template<typename T>
    inline typename long_type_of<T>::type area(const box_t<vec_t<T,2>> &b)
    { return area(b.upper - b.lower); }

    template<typename T>
    inline typename long_type_of<T>::type area(const box_t<vec_t<T,3>> &b)
    {
    ...
    template<typename T>
    inline typename long_type_of<T>::type volume(const box_t<vec_t<T,3>> &b)
    {
      const vec_t<T,3> diag = b.upper - b.lower;
    ...
    template<typename T>
    inline box_t<T> intersection(const box_t<T> &a, const box_t<T> &b)
    { return box_t<T>(max(a.lower,b.lower),min(a.upper,b.upper)); }
  
    template<typename T>
    inline bool operator==(const box_t<T> &a, const box_t<T> &b)
    { return a.lower == b.lower && a.upper == b.upper; }

...
// #endif
      ...
      static inline __both__ QuaternionT<scalar_t> rotation(const LinearSpace3 &a) {
        ...
        if (tr > 1) {
          scalar_t s = cutee::common::polymorphic::sqrt(tr) * 2;
          return QuaternionT<scalar_t>(.25f * s,
                                       (a.vz.y-a.vy.z)/s,
                                       (a.vx.z-a.vz.x)/s,
                                       (a.vy.x-a.vx.y)/s);
        } else if (arg_min(diag) == 0) {
          scalar_t s = cutee::common::polymorphic::sqrt(1.f+diag.x-diag.y-diag.z)*2.f;
          return QuaternionT<scalar_t>((a.vz.y-a.vy.z)/s,
                                       .25f * s,
                                       (a.vx.y-a.vy.x)/s,
                                       (a.vx.z-a.vz.x)/s);
        } else if (arg_min(diag) == 1) {
          scalar_t s = cutee::common::polymorphic::sqrt(1.f+diag.y-diag.x-diag.z)*2.f;
          return QuaternionT<scalar_t>((a.vx.z-a.vz.x)/s,
                                       (a.vx.y-a.vy.x)/s,
                                       .25f * s,
                                       (a.vy.z-a.vz.y)/s);
        } else {
          scalar_t s = cutee::common::polymorphic::sqrt(1.f+diag.z-diag.x-diag.y)*2.f;
